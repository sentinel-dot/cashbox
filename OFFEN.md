...
| A3 | ~~**addItem: Modifier-INSERTs laufen nach dem Commit**~~ — **erledigt 2026-07-22 (S09):** Der INSERT liegt jetzt in `services/orderItemModifiers.ts` (mockbar); scheitert er, kompensiert `addItem` unter dem Order-Lock mit einem `order_item_removals`-Eintrag (kein DELETE — GoBD) und antwortet 500. Damit blendet jede Query die halbfertige Position aus, der Client-Retry erzeugt kein Duplikat mehr. Ist die Bestellung inzwischen nicht mehr offen (Position steht schon auf einem Bon), wird nichts entfernt, sondern laut nach Sentry gemeldet. Das Grundproblem (zwei DB-User ⇒ keine gemeinsame TX) bleibt bauartbedingt — wie bei A9 ist es nur nicht mehr stillschweigend | ✅ |
...
| A6 | ~~**`audit_insert_user` hat INSERT auf alle Tabellen**~~ — **erledigt 2026-07-22 (S09):** `scripts/setup-db.ts` vergibt tabellen-scoped INSERT-Grants auf genau die 6 append-only-Tabellen (`AUDIT_INSERT_TABLES`) und entzieht ein evtl. vorhandenes datenbankweites INSERT. Regressionsschutz: `integration/db-grants.test.ts` (SHOW GRANTS + funktionaler Beweis, dass ein INSERT in `orders` scheitert). Prod-DBA-Anweisung: `docs/betrieb.md` §7 | ✅ |
...
| B3 | ~~**Passwort-Reset**~~ | **Erledigt 2026-07-22 (S08):** `POST /auth/forgot-password` (immer 200, Tenant aus dem Gerät), `GET`+`POST /auth/reset-password` als server-gerenderte Seite ohne JavaScript — **der einzige HTML-Endpunkt des Backends**, weil es kein Web-Frontend gibt und der Link auf jedem Gerät funktionieren muss. Token: 32 Byte, nur SHA-256 in der DB (V013), 1 h, einmalig, neuer Link entwertet den alten; Doppel-Submit über `FOR UPDATE` serialisiert. Zwei Missbrauchsbremsen (IP-Rate-Limit + 3 Mails/Nutzer/Stunde), Token wird aus Logs redigiert (`redactUrl`). **Entscheidung:** Reset beendet ältere Sitzungen über `users.password_changed_at` (`/auth/refresh` → 401). iOS: „Passwort vergessen"-Sheet im LoginView. Vertrag: `docs/api.md`. **Betrieb: `PUBLIC_API_URL` in Prod setzen**, sonst zeigt der Link ins Leere | ✅ |
...
| B8 | ~~**A3 + A6**~~ aus dem Audit (A9 erledigt in S07) | **Erledigt 2026-07-22 (S09):** Kompensation in `addItem` + tabellen-scoped Audit-Grants — Details bei A3/A6 oben | ✅ |
...
| T11 | **`password_reset_tokens` wächst unbegrenzt** | Verbrauchte und abgelaufene Token bleiben stehen (S08 löscht bewusst nichts: `used_at` macht „Link schon benutzt" von „Link unbekannt" unterscheidbar und belegt, wann zurückgesetzt wurde). Bei Reset-Volumen einer Handvoll Betriebe irrelevant, aber sauber wäre ein Cron, der Zeilen älter als 30 Tage löscht. Braucht dann einen `GRANT DELETE` auf genau diese Nicht-Finanztabelle in `scripts/setup-db.ts` (Liste `DELETABLE_TABLES`) | Nach Go-live |
...
2. **Vor Go-live (Reihenfolge):** ~~B1 E-Mail~~ (Code fertig, externes Domain-Gate offen) → ~~B2 Cron~~ → ~~B3 Passwort-Reset~~ → ~~B6/B7~~ → ~~A3/A6~~ (~~A9~~) → B4/B5 → S1/S6 → N1–N9 parallel (Rechtliches/Fiskaly/ELSTER haben Vorlauf!)
