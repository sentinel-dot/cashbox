...
| T10 | **Testläufe melden echte Events ans Produktions-Sentry** | `src/sentry.ts` ruft `dotenv.config()` ohne Pfad — das lädt immer `.env`, nie `.env.test`. Lokal ist dort seit S03 ein echter DSN gesetzt, also loggt jeder `npm test`-Lauf „Sentry aktiv (environment: test)" und schickt tatsächlich Events ins EU-Projekt (aufgefallen bei S05: der `failed`-Pfad der E-Mail-Queue ruft `captureException`). **In CI harmlos** (kein `.env`, DSN leer → No-Op), aber lokal verfälscht es das Dashboard und verbraucht Quota — genau das Dashboard, auf dem später die Alert-Regel sitzt. Fix: in `sentry.ts` denselben Pfad-Switch wie in `db/index.ts` (`NODE_ENV === 'test' ? '.env.test' : '.env'`), plus ein `it()`, das bei `NODE_ENV=test` `sentryEnabled === false` erwartet | Vor Go-live (klein) |
